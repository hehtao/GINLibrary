...
- (CGFloat)dt_cacheSize {
    ...
    [enumerator.allObjects enumerateObjectsUsingBlock:^(NSString *subPath, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        weakSelf.cacheSize += [self dt_fileSizeAtPath:path];
    }];
...
- (CGFloat)dt_folderSizeAtPath:(NSString *)path {
    ...
    if ([kFileManager fileExistsAtPath:path]) {
        ...
        [enumerator.allObjects enumerateObjectsUsingBlock:^(NSString *subPath, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            blockFolderSize += [self dt_fileSizeAtPath:absolutePath];
        }];
        return folderSize;
    }
    return 0;
...
+ (void)gin_wipeCacheWithDefaultStyle {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        UIAlertController *alterVC = [UIAlertController alertControllerWithTitle:@"清除缓存" message:[NSString stringWithFormat:@"当前的缓存为:%.2fM",[[weakSelf dt_sharedManager] dt_cacheSize]] preferredStyle:UIAlertControllerStyleActionSheet];
        UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
            [kTopStackController dismissViewControllerAnimated:YES completion:nil];
        }];
        UIAlertAction *confirmAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
            [[weakSelf dt_sharedManager] dt_wipeCacheAction];
            ...
            // 清空最近浏览记录
          [[NSNotificationCenter defaultCenter] postNotificationName:k_RecentViewServicesNotification object:nil userInfo:@{@"userInfo":@"清除缓存"}];
//            DTPostOrderChangeNotificationWithInfo(@"清空最近浏览记录");
        }];
